import { Plus, TrendingUp, TrendingDown, Wallet, IndianRupee, Calendar, Tag, Filter, Search, Eye, EyeOff, ChevronDown, Trash2, Download } from "lucide-react";
      ...
      <main className="flex-1 w-full p-6 mb-40">
        <div className="max-w-6xl mx-auto">
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
            <h2 className="text-4xl font-black bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">Dashboard</h2>
          ...
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ animationDelay: "1s" }}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-y-3">
              <h3 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
                ...
                Recent Transactions
                <span className="text-sm font-normal text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{filteredTransactions.length}</span>
              </h3>
                ...
                <div
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors duration-300">{transaction.note}</div>
                    ...
                    <div className="text-right flex items-center justify-end">
                      <div className={`text-2xl font-black ${transaction.type === "Expense" ? "text-red-600" : "text-green-600"} group-hover:scale-110 transition-transform duration-300`}>
                        {transaction.type === "Expense" ? "-" : "+"}
                        {formatCurrency(transaction.amount)}
                      </div>
                      <button onClick={() => handleDelete(transaction.id)} className="ml-2 text-gray-400 hover:text-red-600 transition-colors" title="Delete">
                  ...
                  <div className="mt-4 h-1 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-1000 ${transaction.type === "Income" ? "bg-green-400" : "bg-red-400"}`}
                      style={{
